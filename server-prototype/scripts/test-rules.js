...
test('createGame — красная колода собрана отдельным стеком (14 карт)', () => {
  const g = freshGame();
  assert.equal(g.redDeck.length, 14); // 2+2+1 зверей + 3+3 шкур + 3 особых
  assert.ok(g.deck.length > 0); // общая колода не пострадала
});

test('красная клетка — зверь сверху колоды начинает схватку', () => {
  const g = freshGame();
  const { char, result } = stepOnEventCell(g, ['wolf', 'hide_red']);
  ...
  assert.deepEqual(g.redDeck, ['hide_red']);
});

test('красная клетка — не-зверь уходит в инвентарь персонажа', () => {
  const g = freshGame();
  const { char, result } = stepOnEventCell(g, ['hide_red']);
  assert.equal(char.beastFight, null);
  assert.ok(char.inventory.includes('hide_red'));
  assert.equal(result.redEvent.cardId, 'hide_red');
  assert.equal(result.redEvent.beast, false);
  assert.equal(result.redEvent.toInventory, true);
});

test('красная клетка — при полном инвентаре карта уходит в сброс', () => {
  const g = freshGame();
  const char = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  while (char.inventory.length < 10) char.inventory.push('карта');
  const { result } = stepOnEventCell(g, ['hide_red']);
  assert.equal(result.redEvent.cardId, 'hide_red');
  assert.equal(result.redEvent.beast, false);
  assert.equal(result.redEvent.discarded, true);
  assert.ok(g.discard.includes('hide_red'));
});

test('красная клетка — пустая красная колода: события нет', () => {
  const g = freshGame();
  const { char, result } = stepOnEventCell(g, []);
  assert.equal(char.beastFight, null);
  assert.equal(result.redEvent.empty, true);
});
...
test('transfer (ящик) — на разных клетках отклоняется', () => {
  const g = freshGame();
  rollAndSplit(g, 'p1');
  const k = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  const o = g.characters.find(c => c.owner === 'p1' && c.role === 'O');
  // Дефолтные стартовые позиции K и O разные — берём как есть
...
test('transfer (ящик) — на соседней клетке проходит', () => {
  const g = freshGame();
  rollAndSplit(g, 'p1');
  const k = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  const o = g.characters.find(c => c.owner === 'p1' && c.role === 'O');
  o.position = neighbors(k.position)[0]; // ставим рядом
  o.inventory = [];
...
test('transfer (легаси) — на разных клетках отклоняется', () => {
  const g = freshGame();
  rollAndSplit(g, 'p1');
  const k = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  const o = g.characters.find(c => c.owner === 'p1' && c.role === 'O');
...
test('move (split) — тот же персонаж может ходить обоими кубиками', () => {
  const g = freshGame();
  apply(g, 'p1', 'turn:roll');
  ...
  const t1 = availableMoveTargets(g, 'p1', v.id, 0)[0];
  apply(g, 'p1', 'action:move', { characterId: v.id, toCell: t1.cellId, dieIndex: 0 });
  const t2 = availableMoveTargets(g, 'p1', v.id, 1)[0];
  assert.ok(t2, 'у того же персонажа должны быть цели вторым кубиком');
  apply(g, 'p1', 'action:move', { characterId: v.id, toCell: t2.cellId, dieIndex: 1 });
});
