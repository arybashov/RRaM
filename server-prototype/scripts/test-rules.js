...
import { neighbors, startCell, shortestDistance, cellTerrain } from '../src/map.js';
...
test('fightBeast — kill даёт «Шкуру убитого зверя», туша зверя в сброс', () => {
  ...
  assert.equal(result.beastFought.hide, 'raw_hide_red'); // волк — красный зверь
  assert.equal(warrior.beastFight, null);
  assert.ok(warrior.inventory.includes('raw_hide_red'));
...
test('processHide — шаман превращает сырую шкуру в очищенную (кубик ≥2)', () => {
  const g = freshGame();
  const shaman = g.characters.find(c => c.owner === 'p1' && c.role === 'S');
  shaman.inventory.push('raw_hide_red');
  g.turn.dice = [3, 1];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
  ...
  assert.equal(result.hideProcessed.cleaned, 'hide_red');
  assert.ok(shaman.inventory.includes('hide_red'));
...
test('craft Молоток — открывается смешанной рудой (кузнец), без иглы', () => {
  const g = freshGame();
  const smith = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  smith.inventory.push('ore_medium');
...
test('craft Мешок — клубок + очищенная шкура барана (помощник), без иглы', () => {
  ...
  helper.inventory.push('sheep_hide_c'); // клубок (yarn) уже базовый
