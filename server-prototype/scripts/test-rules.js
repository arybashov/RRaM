...
test('draw — после постановки фишки на ресурс фиксирует выбор клетки', () => {
  const g = freshGame();
  const helper = g.characters.find(c => c.owner === 'p1' && c.role === 'P');
  ...
  assert.equal(g.turn.movementArea.locked, false);
...
test('автосплит — выход на ресурс в пределах одного кубика тратит наименьший, больший свободен, добор доступен', () => {
  const g = freshGame();
  g.turn.dice = [2, 5];
  g.turn.mode = 'moveSum';
  const warrior = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  ...
  apply(g, 'p1', 'action:move', { characterId: warrior.id, toCell: 'H014' });
