...
function activateSplitTurn(game, playerId, character, dice = [3, 4]) {
...
test('draw/transfer/terrain - card instance keeps source deck back', () => {
  ...
  apply(g, 'p1', 'action:draw', { characterId: from.id, dieIndex: 0 });
...
test('dwarves — настроены на выход после 5-го полного хода игроков', () => {
  const g = freshGame();
  ...
  assert.equal(g.dwarves.units.length, 5);
  assert.ok(g.dwarves.units.every(unit => unit.position === null));
});

test('dwarves — выходят из ворот после пятого полного хода игроков', () => {
  const g = freshGame();
  ...
  for (let round = 1; round < 5; round += 1) {
    ...
    const p2End = apply(g, 'p2', 'turn:end');
    assert.equal(p2End.dwarves, null);
    assert.equal(g.dwarves.mainTurnsCompleted, round);
    assert.equal(g.dwarves.active, false);
    assert.ok(g.dwarves.units.every(unit => unit.position === null));
  }
  ...
  assert.equal(g.dwarves.mainTurnsCompleted, 5);
...
test('dwarves — после немедленного выхода идут по следующей точке маршрута', () => {
  const g = freshGame();
  const route = dwarfRoute();
  for (const character of g.characters) character.position = null;
  g.dwarves.entryTurn = 0; // активируем сразу — тест проверяет движение, а не таймер выхода
  __setDwarfDiceRoller(() => [1, 0]); // дальность 1 клетка — проверяем пошаговую механику
  const skipped = apply(g, 'p1', 'turn:end');
  assert.equal(skipped.dwarves, null);

  const first = apply(g, 'p2', 'turn:end');
  assert.equal(first.dwarves.type, 'dwarfTurn');
  assert.equal(first.dwarves.entries.length, 5);
  assert.deepEqual(first.dwarves.entries.map(entry => entry.toCell), route.slice(0, 5));
  assert.deepEqual(first.dwarves.moves, []);
  assert.deepEqual(g.dwarves.units.map(unit => unit.position), route.slice(0, 5));
  assert.equal(new Set(g.dwarves.units.map(unit => unit.position)).size, 5);

  const skippedAgain = apply(g, 'p1', 'turn:end');
  assert.equal(skippedAgain.dwarves, null);

  const second = apply(g, 'p2', 'turn:end');
  assert.equal(second.dwarves.type, 'dwarfTurn');
  assert.equal(second.dwarves.moves.length, 5);
  assert.deepEqual(second.dwarves.moves.map(move => move.toCell).sort(), route.slice(1, 6).sort());
  assert.deepEqual(second.dwarves.entries, []);
  assert.deepEqual(g.dwarves.units.map(unit => unit.position), route.slice(1, 6));
  assert.equal(new Set(g.dwarves.units.map(unit => unit.position)).size, 5);
  __setDwarfDiceRoller();
});

test('dwarves - держат направление по повторяющимся клеткам, у конца маршрута уходят с поля', () => {
  const g = freshGame();
  const route = dwarfRoute();
  for (const character of g.characters) character.position = null;
  g.dwarves.entryTurn = 0;
  __setDwarfDiceRoller(() => [1, 0]);

  const startIndex = route.length - g.dwarves.units.length;
  g.dwarves.units.forEach((unit, offset) => {
    const routeIndex = startIndex + offset;
    unit.position = route[routeIndex];
    unit.routeIndex = routeIndex;
  });

  // route[startIndex] — повторяющаяся клетка: indexOf даёт первое вхождение, а не наш индекс.
  assert.notEqual(route.indexOf(g.dwarves.units[0].position), g.dwarves.units[0].routeIndex);

  apply(g, 'p1', 'turn:end');
  const result = apply(g, 'p2', 'turn:end');

  assert.equal(result.dwarves.type, 'dwarfTurn');
  // 4 шага вперёд + 1 выход с поля (последний дварф у ворот).
  assert.equal(result.dwarves.moves.length, 5);
  const exitMove = result.dwarves.moves.find(m => m.exit);
  assert.ok(exitMove, 'должен быть ход-выход');

  const last = g.dwarves.units[4];
  assert.equal(last.exited, true);
  assert.equal(last.position, null);

  const stayed = g.dwarves.units.slice(0, 4);
  assert.deepEqual(stayed.map(u => u.routeIndex), [110, 111, 112, 113]);
  assert.deepEqual(stayed.map(u => u.position), [route[110], route[111], route[112], route[113]]);
  assert.equal(new Set(stayed.map(u => u.position)).size, 4);
  __setDwarfDiceRoller();
});

test('dwarves — пройдя весь маршрут, уходят с поля и не возвращаются', () => {
  const g = freshGame();
  const route = dwarfRoute();
  for (const character of g.characters) character.position = null;
  g.dwarves.entryTurn = 0;
  __setDwarfDiceRoller(() => [6, 6]); // быстрый прогон до ворот

  // Гоняем раунды, пока все дварфы не уйдут с поля.
  let guard = 0;
  while (g.dwarves.units.some(u => !u.exited) && guard < 200) {
    apply(g, 'p1', 'turn:end');
    apply(g, 'p2', 'turn:end');
    guard += 1;
  }

  assert.ok(g.dwarves.units.every(u => u.exited), 'все дварфы должны уйти с поля');
  assert.ok(g.dwarves.units.every(u => u.position === null), 'ушедших нет на поле');

  // Дальше они не появляются снова.
  apply(g, 'p1', 'turn:end');
  const after = apply(g, 'p2', 'turn:end');
  assert.ok(g.dwarves.units.every(u => u.position === null));
  assert.ok(after.dwarves == null || after.dwarves.type !== 'dwarfTurn' || (after.dwarves.entries ?? []).length === 0);
  __setDwarfDiceRoller();
});

test('dwarves - off-route unit recovers by route index direction', () => {
  const g = freshGame();
  const route = dwarfRoute();
  for (const character of g.characters) character.position = null;
  g.dwarves.entryTurn = 0;
  ...
  const routeIndex = 20;
  const offRoute = neighbors(route[routeIndex])
    .find(cellId => !route.includes(cellId) && shortestDistance(cellId, route[routeIndex + 1]) < Infinity);
  assert.ok(offRoute);

  g.dwarves.units.forEach((unit, offset) => {
    unit.position = route[routeIndex + offset + 1];
    unit.routeIndex = routeIndex + offset + 1;
  });
  const unit = g.dwarves.units[0];
  unit.position = offRoute;
  unit.routeIndex = routeIndex;

  const beforeDistance = shortestDistance(unit.position, route[routeIndex + 1]);
  apply(g, 'p1', 'turn:end');
  const result = apply(g, 'p2', 'turn:end');
  const move = result.dwarves.moves.find(item => item.unitId === unit.id);

  assert.ok(move);
  assert.equal(move.routeRecovery, true);
  assert.ok(shortestDistance(unit.position, route[routeIndex + 1]) < beforeDistance);
  assert.ok([routeIndex, routeIndex + 1].includes(unit.routeIndex));
  __setDwarfDiceRoller();
});

test('dwarves - one-step follower sidesteps when the next route cell is occupied', () => {
  const g = freshGame();
  const route = dwarfRoute();
  for (const character of g.characters) character.position = null;
  g.dwarves.entryTurn = 0;
  ...
  const mover = g.dwarves.units[0];
  const blocker = g.dwarves.units[1];
  mover.position = route[20];
  mover.routeIndex = 20;
  blocker.position = route[21];
  blocker.routeIndex = -1; // stationary unit: occupies the next route cell but does not move this turn

  apply(g, 'p1', 'turn:end');
  const result = apply(g, 'p2', 'turn:end');
  const move = result.dwarves.moves.find((item) => item.unitId === mover.id);

  assert.ok(move);
  assert.notEqual(mover.position, route[20]);
  assert.notEqual(mover.position, route[21]);
  assert.equal(shortestDistance(mover.position, route[22]), 1);
  assert.equal(blocker.position, route[21]);
  __setDwarfDiceRoller();
});

test('dwarves - aggro does not move backward by route numbers', () => {
  const g = freshGame();
  const route = dwarfRoute();
  for (const character of g.characters) character.position = null;
  g.dwarves.entryTurn = 0;
  ...
  unit.position = route[10];
  unit.routeIndex = 10;

  const victim = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  victim.position = route[8];
  victim.hp = 100;
  assert.ok(shortestDistance(unit.position, victim.position) > 1);
  assert.ok(shortestDistance(unit.position, victim.position) <= 5);
  ...
  assert.ok(move);
  assert.equal(move.aggroTargetId, undefined);
  assert.equal(unit.routeIndex, 11);
  assert.equal(unit.position, route[11]);
  __setDwarfDiceRoller();
});

test('dwarves — за ход проходят вдоль маршрута на сумму своих кубиков', () => {
  const g = freshGame();
  const route = dwarfRoute();
  for (const character of g.characters) character.position = null;
  g.dwarves.entryTurn = 0;
  __setDwarfDiceRoller(() => [3, 4]); // дальность 7 клеток

  const unit = g.dwarves.units[0];
  g.dwarves.units.forEach((dwarf) => { dwarf.position = null; dwarf.routeIndex = -1; });
  unit.position = route[10];
  unit.routeIndex = 10;
  ...
  assert.ok(move);
  assert.deepEqual(move.dice, [3, 4]);
  assert.equal(move.steps, 7);
  assert.equal(unit.routeIndex, 17);
  assert.equal(unit.position, route[17]);
  __setDwarfDiceRoller();
});

test('dwarves — останавливаются у цели в дальности атаки, не проходя мимо', () => {
  const g = freshGame();
  const route = dwarfRoute();
  for (const character of g.characters) character.position = null;
  g.dwarves.entryTurn = 0;
  __setDwarfDiceRoller(() => [6, 6]); // дальность 12, но должен встать у цели

  const unit = g.dwarves.units[0]; // обычный дварф, дальность атаки 1
  g.dwarves.units.forEach((dwarf) => { dwarf.position = null; dwarf.routeIndex = -1; });
  unit.position = route[10];
  unit.routeIndex = 10;

  // Цель рядом с route[12] — дварф дойдёт до неё и встанет (≤ дальности атаки).
  const victim = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  victim.position = route[12];
  victim.hp = 100;

  apply(g, 'p1', 'turn:end');
  const result = apply(g, 'p2', 'turn:end');

  // Дошёл максимум до соседней с целью клетки (route[11]) и не пошёл дальше route[12].
  assert.ok(shortestDistance(unit.position, victim.position) <= 1);
  assert.ok(unit.routeIndex <= 12);
...
test('dwarves — сейчас не получают игровые кубики персонажей', () => {
  const g = freshGame();
...
test('dwarves — атакуют персонажа рядом вместо шага по маршруту', () => {
  ...
  apply(g, 'p2', 'turn:end');
  const victim = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  const occupied = new Set(route.slice(0, 5));
  victim.position = neighbors(route[0]).find(cellId => !occupied.has(cellId));
  victim.hp = 100;
  ...
  assert.ok(attack);
  assert.ok([10, 15].includes(attack.damage));
  assert.ok(victim.hp < 100);
});
...
test('dwarves — стрелок атакует в радиусе 5 клеток', () => {
  ...
  apply(g, 'p1', 'turn:end');
  apply(g, 'p2', 'turn:end');
  const victim = g.characters.find(c => c.owner === 'p1' && c.role === 'O');
  const occupied = new Set(route.slice(0, 5));
  const target = reachableCells(route[4], 5)
    .find(({ cellId, distance }) =>
      distance > 1
      && !occupied.has(cellId)
      && route.slice(0, 5).every(routeCell => shortestDistance(cellId, routeCell) > 1));
  assert.ok(target);
  victim.position = target.cellId;
  victim.hp = 100;

  apply(g, 'p1', 'turn:end');
  const result = apply(g, 'p2', 'turn:end');
  const attack = result.dwarves.attacks.find(item =>
    item.targetId === victim.id && item.distance > 1);

  assert.ok(attack);
  assert.equal(attack.damage, 15);
  assert.ok(victim.hp <= 85);
});
...
test('Дубовые желуди — на точке добычи показывают 2 карты и дают выбрать 1', () => {
  const g = freshGame();
  const warrior = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  warrior.inventory.push('art_forest_003');
  warrior.position = 'H014';
  g.deck = ['ore_medium', 'art_mixed_003', 'ore_coarse'];

  const preview = apply(g, 'p1', 'action:useOakAcorns', {
    characterId: warrior.id,
    cardIndex: warrior.inventory.indexOf('art_forest_003'),
  });

  assert.deepEqual(preview.oakAcornsChoice.cards.map(card => card.card), ['ore_medium', 'art_mixed_003']);
  assert.deepEqual(g.deck, ['ore_medium', 'art_mixed_003', 'ore_coarse']);
  assert.ok(!warrior.inventory.includes('ore_medium'));

  const result = apply(g, 'p1', 'action:useOakAcorns', {
    characterId: warrior.id,
    cardIndex: warrior.inventory.indexOf('art_forest_003'),
    choiceIndex: 1,
  });

  assert.equal(result.oakAcornsUsed.card, 'art_mixed_003');
  assert.equal(result.oakAcornsUsed.discarded, 'ore_medium');
  assert.ok(warrior.inventory.includes('art_mixed_003'));
  assert.ok(warrior.inventory.includes('art_forest_003'));
  assert.deepEqual(g.deck, ['ore_coarse']);
  assert.ok(g.discard.includes('ore_medium'));
});

test('Дубовые желуди — повторно доступны через 4 сброса кубиков', () => {
  const g = freshGame();
  const warrior = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  warrior.inventory.push('art_forest_003');
  warrior.position = 'H014';
  g.deck = ['ore_medium', 'art_mixed_003', 'ore_coarse', 'art_mixed_001'];
  g.turn.rollsLeft.p1 = 9;

  apply(g, 'p1', 'action:useOakAcorns', {
    characterId: warrior.id,
    cardIndex: warrior.inventory.indexOf('art_forest_003'),
    choiceIndex: 0,
  });

  assert.equal(warrior.oakAcornsReadyRoll, 5);
  assert.throws(
    () => apply(g, 'p1', 'action:useOakAcorns', {
      characterId: warrior.id,
      cardIndex: warrior.inventory.indexOf('art_forest_003'),
  ...
  g.turn.rollsLeft.p1 = 5;
  const preview = apply(g, 'p1', 'action:useOakAcorns', {
    characterId: warrior.id,
    cardIndex: warrior.inventory.indexOf('art_forest_003'),
  });
  assert.deepEqual(preview.oakAcornsChoice.cards.map(card => card.card), ['ore_coarse', 'art_mixed_001']);
});
...
test('snapshot — соперник не видит значения брошенных кубиков', () => {
  const g = freshGame();
  apply(g, 'p1', 'turn:roll');

  const own = snapshotGame(g, 'p1');
  const enemy = snapshotGame(g, 'p2');

  assert.ok(Object.keys(own.turn.diceByCharacter).length > 0);
  assert.equal(enemy.turn.hasRolled, true);
  assert.equal(enemy.turn.dice, null);
  assert.deepEqual(enemy.turn.diceByCharacter, {});
  assert.deepEqual(enemy.turn.usedDiceByCharacter, {});
});
...
for (const playerId of ['p1', 'p2']) {
  test(`draw - works for ${playerId}`, () => {
    const g = freshGame();
    const char = g.characters.find(c => c.owner === playerId && c.role === 'V');
    char.position = 'H014';
    char.inventory = [];
    g.deck = ['ore_medium'];
    activateSplitTurn(g, playerId, char, [3, 4]);

    const result = apply(g, playerId, 'action:draw', { characterId: char.id, dieIndex: 0 });
    ...
    assert.deepEqual(g.turn.usedDiceByCharacter[char.id], [true, false]);
  });
}
...
test('drawProfession — кузнец по кнопке берет настоящий чертеж', () => {
  const g = freshGame();
  const blacksmith = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  g.decks.blueprints = ['art_dark_forest_003'];
  g.turn.dice = [3, 4];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
...
test('draw — карта зверя из обычной колоды начинает схватку и не попадает в инвентарь', () => {
  ...
  const char = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  char.position = 'H011';
  g.decks.forest = ['boar_forest', 'bark'];
  g.turn.dice = [3, 4];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
  ...
  assert.equal(result.drawn.card, 'boar_forest');
  assert.equal(result.drawn.beast, true);
  assert.deepEqual(char.beastFight, { cardId: 'boar_forest', successes: 0, cellId: 'H011' });
  assert.ok(!char.inventory.includes('boar_forest'));
  assert.deepEqual(g.decks.forest, ['bark']);
});
...
test('move — после постановки фишки на ресурс автоматически добирает и фиксирует выбор клетки', () => {
  ...
  g.decks.sheep = ['sheep_wool', ...g.decks.sheep.filter(cardId => cardId !== 'sheep_wool')];
  g.turn.dice = [1, 4];
  g.turn.usedDice = [false, false];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
  ...
  assert.equal(result.drawn.card, 'sheep_wool');
...
test('draw — ресурсный добор не кладёт карты Ирикона в руку или сброс', () => {
  ...
  const result = apply(g, 'p1', 'action:draw', { characterId: char.id, dieIndex: 0 });
  const forbidden = ['irikon', 'task_irikon'];
...
for (const playerId of ['p1', 'p2']) {
  test(`transfer by cardIndex - works for ${playerId}`, () => {
    const g = freshGame();
    const from = g.characters.find(c => c.owner === playerId && c.role === 'K');
    const to = g.characters.find(c => c.owner === playerId && c.role === 'O');
    from.inventory = ['ore_medium', 'bark', 'yarn'];
    to.inventory = [];
    activateSplitTurn(g, playerId, from, [3, 4]);

    const result = apply(g, playerId, 'action:transfer', {
      fromId: from.id,
      toId: to.id,
      cardIndex: 1,
      dieIndex: 0,
    ...
    assert.equal(result.transferred.cardId, 'bark');
    assert.deepEqual(from.inventory, ['ore_medium', 'yarn']);
    assert.deepEqual(to.inventory, ['bark']);
    assert.deepEqual(g.turn.usedDiceByCharacter[from.id], [true, false]);
  });
}
...
test('автосплит — выход на ресурс в пределах одного кубика тратит наименьший и автоматически добирает карту', () => {
  ...
  assert.equal(g.turn.modeByCharacter[warrior.id], 'split'); // автосплит — персональный режим персонажа
  ...
  assert.deepEqual(g.turn.drawnCharacterIdsThisTurn, [warrior.id]);
...
test('move split — другой кубик открывает вторую ногу от текущей клетки', () => {
  ...
  const firstLeg = availableMoveTargets(g, 'p1', warrior.id, 0)
    .find((target) => {
      const trial = freshGame();
      trial.turn.dice = [3, 3];
      trial.turn.mode = 'split';
      const trialWarrior = trial.characters.find(c => c.owner === 'p1' && c.role === 'V');
      apply(trial, 'p1', 'action:move', { characterId: trialWarrior.id, toCell: target.cellId, dieIndex: 0 });
      return availableMoveTargets(trial, 'p1', trialWarrior.id, 1).some(t => t.distance === 3);
    });
  assert.ok(firstLeg);
...
test('move - own unit is passable but not a valid stop cell', () => {
  const g = freshGame();
  const warrior = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  const friend = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  for (const character of g.characters) {
    if (character !== warrior && character !== friend) character.position = null;
  }

  const start = warrior.position;
  const mid = neighbors(start).find((candidate) =>
    neighbors(candidate).some((next) => next !== start && !neighbors(start).includes(next)));
  const dest = neighbors(mid).find((candidate) =>
    candidate !== start && !neighbors(start).includes(candidate));
  friend.position = mid;
  g.turn.dice = [2, 1];
  g.turn.mode = 'split';
  ...
  assert.ok(!targets.some((target) => target.cellId === mid));
  assert.ok(targets.some((target) => target.cellId === dest));
});

test('move - enemy unit blocks passage and stop cell', () => {
  const g = freshGame();
  const warrior = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  const enemy = g.characters.find(c => c.owner === 'p2' && c.role === 'K');
  for (const character of g.characters) {
    if (character !== warrior && character !== enemy) character.position = null;
  }
...
test('teleport — переносит персонажа на свободную стартовую клетку', () => {
  ...
  g.turn.mode = 'split';
  const shaman = g.characters.find(c => c.owner === 'p1' && c.role === 'S');
  const dest = startCell('green', 'S');
  shaman.position = neighbors(dest).find(
    (cellId) => !g.characters.some((character) => character.position === cellId),
  );
...
test('teleport — второй игрок переносится на старт своей роли', () => {
  const g = freshGame();
  g.turn.activePlayerId = 'p2';
  g.turn.dice = [3, 1];
  g.turn.mode = 'split';
  const shaman = g.characters.find(c => c.owner === 'p2' && c.role === 'S');
  const dest = startCell('red', 'S');
  shaman.position = neighbors(dest).find(
    (cellId) => !g.characters.some((character) => character.position === cellId),
  );
...
test('teleport — старт другого персонажа своей команды запрещён', () => {
  const g = freshGame();
  g.turn.dice = [3, 1];
  g.turn.mode = 'split';
  const shaman = g.characters.find(c => c.owner === 'p1' && c.role === 'S');
...
for (const playerId of ['p1', 'p2']) {
  test(`terrain place/flip/remove - works for ${playerId}`, () => {
    const g = freshGame();
    const warrior = g.characters.find(c => c.owner === playerId && c.role === 'V');
    warrior.inventory = ['bark', 'teleport_beads'];
    g.turn.activePlayerId = playerId;

    const placed = apply(g, playerId, 'action:terrainPlace', {
      id: `terrain-${playerId}`,
      characterId: warrior.id,
      cardIndex: 0,
      x: 100,
    ...
    assert.equal(placed.terrainPlaced.cardId, 'bark');
    assert.deepEqual(warrior.inventory, ['teleport_beads']);
    assert.equal(g.terrainCards[0].ownerId, playerId);
    assert.equal(g.terrainCards[0].characterId, warrior.id);
    assert.equal(g.terrainCards[0].faceDown, true);

    const flipped = apply(g, playerId, 'action:terrainFlip', {
      id: `terrain-${playerId}`,
      faceDown: false,
    });
    assert.equal(flipped.terrainFlipped.faceDown, false);
    assert.equal(g.terrainCards[0].faceDown, false);

    const removed = apply(g, playerId, 'action:terrainRemove', { id: `terrain-${playerId}` });
    assert.equal(removed.terrainRemoved.cardId, 'bark');
    assert.deepEqual(warrior.inventory, ['bark', 'teleport_beads']);
    assert.equal(g.terrainCards.length, 0);
  });
}

test('terrain — вверх ногами хранится отдельно от рубашки вверх', () => {
  const g = freshGame();
  const warrior = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  warrior.inventory = ['bark', 'teleport_beads'];

  const placed = apply(g, 'p1', 'action:terrainPlace', {
    id: 'terrain-upside-down',
    characterId: warrior.id,
    cardIndex: 0,
    x: 100,
    y: 200,
  ...
  assert.equal(placed.terrainPlaced.faceDown, false);
  assert.equal(placed.terrainPlaced.upsideDown, true);

  const closed = apply(g, 'p1', 'action:terrainFlip', {
    id: 'terrain-upside-down',
    faceDown: true,
  });
  assert.equal(closed.terrainFlipped.faceDown, true);
  assert.equal(closed.terrainFlipped.upsideDown, false);

  const openedUpsideDown = apply(g, 'p1', 'action:terrainFlip', {
    id: 'terrain-upside-down',
    faceDown: false,
    upsideDown: true,
  });
  assert.equal(openedUpsideDown.terrainFlipped.faceDown, false);
  assert.equal(openedUpsideDown.terrainFlipped.upsideDown, true);

  const snapshot = snapshotGame(g, 'p1');
  const terrainCard = snapshot.terrainCards.find((card) => card.id === 'terrain-upside-down');
  assert.equal(terrainCard.faceDown, false);
  assert.equal(terrainCard.upsideDown, true);
});
...
test('terrain — Полена дерева можно выложить как боевую карту', () => {
  const g = freshGame();
  const blacksmith = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  blacksmith.inventory = ['art_forest_005'];

  const result = apply(g, 'p1', 'action:terrainPlace', {
    id: 'terrain-firewood',
  ...
  assert.equal(result.terrainPlaced.cardId, 'art_forest_005');
  assert.deepEqual(blacksmith.inventory, []);
  assert.equal(g.terrainCards.length, 1);
  assert.equal(g.terrainCards[0].faceDown, false);
});

test('Полена дерева — в схватке со зверем уменьшают входящий урон на 5', () => {
  const g = freshGame();
  const warrior = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  warrior.inventory.push('art_forest_005');
  apply(g, 'p1', 'action:terrainPlace', {
    id: 'firewood-beast',
    characterId: warrior.id,
    cardIndex: warrior.inventory.indexOf('art_forest_005'),
    x: 100,
    y: 200,
  });
  ...
  assert.equal(warrior.hp, 95);
  assert.equal(g.terrainCards[0].faceDown, false);
});

test('Полена дерева — в драке с противником умножают урон на 2', () => {
  const g = freshGame();
  ...
  attacker.inventory.push('art_forest_005');
  apply(g, 'p1', 'action:terrainPlace', {
    id: 'firewood-pvp',
    characterId: attacker.id,
    cardIndex: attacker.inventory.indexOf('art_forest_005'),
    x: 100,
    y: 200,
  });
  g.turn.dice = [2, 3];
  ...
  assert.equal(result.attacked.firewoodMultiplier, 2);
  assert.equal(result.attacked.totalDamage, 10);
  assert.equal(result.attacked.dealtDamage, 10);
...
test('combat — участник боя не может брать карту, но может телепортироваться', () => {
  ...
  const destination = startCell(target.side, target.role);
  const result = apply(g, 'p2', 'action:teleport', {
    characterId: target.id,
    toCell: destination,
    dieIndex: 0,
...
test('attack — после убийства первой цели можно вторым кубиком ударить другую', () => {
  const g = freshGame();
  const { attacker, target } = prepareAdjacentCombat(g);
  const secondTarget = g.characters.find(c => c.owner === 'p2' && c.role === 'K');
  const secondCell = neighbors(attacker.position)
    .find(cell => cell !== target.position && !g.characters.some(c => c.position === cell));
  assert.ok(secondCell);
  secondTarget.position = secondCell;
  target.hp = 7;

  const first = apply(g, 'p1', 'action:attack', {
    attackerId: attacker.id,
    targetId: target.id,
  });
  assert.equal(first.attacked.defeated, true);
  assert.equal(target.position, null);
  assert.equal(attacker.combatOpponentId, null);
  assert.deepEqual(g.turn.usedDice, [true, false]);
  assert.deepEqual(availableAttackTargets(g, 'p1', attacker.id), [secondTarget.id]);

  const second = apply(g, 'p1', 'action:attack', {
    attackerId: attacker.id,
    targetId: secondTarget.id,
  });
  assert.equal(second.attacked.targetId, secondTarget.id);
  assert.equal(second.attacked.dieIndex, 1);
  assert.equal(second.attacked.damage, 7);
  assert.equal(secondTarget.hp, 93);
  assert.deepEqual(g.turn.usedDice, [true, true]);
});
...
test('createGame — красная колода собирается по JSON-разделителю', () => {
  const g = freshGame();
  assert.equal(g.redDeck.length, 21);
  assert.equal(g.redDeck.includes('irikon'), false);
  assert.equal(g.redDeck.includes('task_irikon'), false);
  assert.equal(g.redDeck.filter(cardId => cardId === 'bear_hide').length, 4);
  assert.equal(g.redDeck.filter(cardId => cardId === 'art_trophy_001').length, 2);
  assert.equal(g.redDeck.filter(cardId => cardId === 'art_trophy_002').length, 2);
  assert.equal(g.redDeck.filter(cardId => cardId === 'boar_red').length, 2);
  assert.equal(g.redDeck.filter(cardId => cardId === 'wolf').length, 1);
  assert.equal(g.redDeck.filter(cardId => cardId === 'raw_hide_red').length, 3);
...
test('красная клетка — бурый медведь начинает схватку и даёт шкуру медведя', () => {
  const g = freshGame();
  const { char, result } = stepOnEventCell(g, ['art_trophy_002']);
  assert.deepEqual(char.beastFight, { cardId: 'art_trophy_002', successes: 0, cellId: EVENT_CELL });
  assert.equal(result.redEvent.cardId, 'art_trophy_002');
  assert.equal(result.redEvent.beast, true);

  g.turn.usedDice = [false, false];
  g.turn.diceByCharacter[char.id] = [5, 2];
  const fight = apply(g, 'p1', 'action:fightBeast', { characterId: char.id, dieIndex: 0 });
  assert.equal(fight.beastFought.killed, true);
  assert.equal(fight.beastFought.hide, 'bear_hide');
...
test('красная клетка — Ирикон не выпадает даже при редком броске', () => {
  const g = freshGame();
  const originalRandom = Math.random;
  Math.random = () => 0;
  try {
    ...
    assert.deepEqual(char.beastFight, { cardId: 'wolf', successes: 0, cellId: EVENT_CELL });
    assert.equal(result.redEvent.cardId, 'wolf');
    assert.equal(result.redEvent.specialRoll, false);
    assert.equal(result.redEvent.beast, true);
    assert.ok(!char.inventory.includes('irikon'));
    assert.deepEqual(g.redDeck, []);
  } finally {
    Math.random = originalRandom;
  }
});

test('красная клетка — Ирикон не выпадает при повторных красных событиях', () => {
  const g = freshGame();
  const originalRandom = Math.random;
  Math.random = () => 0;
  try {
    const first = stepOnEventCell(g, ['wolf', 'boar_red']).result;
    assert.equal(first.redEvent.cardId, 'wolf');
    ...
    assert.equal(second.redEvent.cardId, 'boar_red');
  } finally {
    Math.random = originalRandom;
  }
});
...
test('красная клетка — пустая колода пересобирается и даёт красное событие', () => {
  ...
  try {
    ...
    assert.ok(['hide_red', 'bear_hide', 'raw_hide_red', 'boar_red', 'wolf', 'art_trophy_001', 'art_trophy_002'].includes(result.redEvent.cardId));
...
test('Топормол — убивает зверя одним любым кубиком и остаётся в инвентаре', () => {
  const g = freshGame();
  const smith = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  smith.beastFight = { cardId: 'beast_bear', successes: 0 };
  smith.inventory.push('topormol');
  g.turn.dice = [1, 2];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
  ...
  assert.equal(result.beastFought.killed, true);
  assert.equal(result.beastFought.topormolUsed, true);
  assert.equal(smith.beastFight, null);
  assert.ok(smith.inventory.includes('topormol'));
  assert.ok(smith.inventory.includes('bear_hide'));
  assert.deepEqual(g.turn.usedDice, [true, false]);
});
...
test('Жаба ворчун — любой персонаж завершает схватку со зверем и карта возвращается', () => {
  const g = freshGame();
  const warrior = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  warrior.inventory.push('art_fairy_glade_005');
  warrior.beastFight = { cardId: 'wolf', successes: 0, cellId: warrior.position };

  const result = apply(g, 'p1', 'action:useLakeFrog', { characterId: warrior.id, cardId: 'art_fairy_glade_005' });

  assert.equal(result.lakeFrogUsed.mode, 'beast');
  assert.equal(result.lakeFrogUsed.cardId, 'art_fairy_glade_005');
  assert.equal(result.lakeFrogUsed.name, 'Жаба ворчун');
  assert.equal(warrior.beastFight, null);
  assert.ok(g.discard.includes('wolf'));
  assert.ok(warrior.inventory.includes('art_fairy_glade_005'));
  assert.ok(warrior.inventory.includes('wolf_hide'));
});

test('Жаба ворчун — против игрока отключает оружие до броска суммы 7+', () => {
  const g = freshGame();
  const warrior = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  const enemySmith = g.characters.find(c => c.owner === 'p2' && c.role === 'K');
  enemySmith.position = neighbors(warrior.position)[0];
  warrior.inventory.push('art_fairy_glade_005');
  enemySmith.inventory.push('irikon');
  ...
  const cast = apply(g, 'p1', 'action:useLakeFrog', {
    characterId: warrior.id,
    targetId: enemySmith.id,
    cardId: 'art_fairy_glade_005',
  });
  assert.equal(cast.lakeFrogUsed.mode, 'player');
  assert.equal(cast.lakeFrogUsed.name, 'Жаба ворчун');
  assert.equal(cast.lakeFrogUsed.dischargeTotal, 7);
  assert.ok(enemySmith.frogSpell);
  assert.equal(enemySmith.frogSpell.cardId, 'art_fairy_glade_005');
  assert.ok(!warrior.inventory.includes('art_fairy_glade_005'));
  ...
  g.turn.usedDice = [false, false];
  g.turn.hasRolled = true;
  const attack = apply(g, 'p2', 'action:attack', { attackerId: enemySmith.id, targetId: warrior.id });

  assert.equal(attack.attacked.weaponDisabledByLakeFrog, true);
  assert.equal(attack.attacked.weaponSuppressedSpellName, 'Жаба ворчун');
  assert.equal(attack.attacked.weaponDamage, 0);
  assert.equal(attack.attacked.totalDamage, 7);
  ...
  try {
    ...
    assert.equal(enemySmith.frogSpell, null);
    assert.ok(warrior.inventory.includes('art_fairy_glade_005'));
    assert.equal(result.roll.lakeFrogReleased[0].name, 'Жаба ворчун');
  } finally {
    Math.random = originalRandom;
  }
});

test('Жаба ворчун — останавливает атаку дварфа до его броска суммы 7+', () => {
  const g = freshGame();
  const warrior = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  const unit = g.dwarves.units[0];
  const targetCell = g.dwarves.route[0];
  unit.position = targetCell;
  unit.routeIndex = 0;
  warrior.position = neighbors(targetCell)[0];
  g.dwarves.active = true;
  g.dwarves.mainTurnsCompleted = 1;
  warrior.inventory.push('art_fairy_glade_005');
  ...
  assert.equal(cast.lakeFrogUsed.mode, 'dwarf');
  assert.equal(unit.frogSpell?.name, 'Жаба ворчун');
  assert.ok(!warrior.inventory.includes('art_fairy_glade_005'));

  g.turn.activePlayerId = 'p2';
  __setDwarfDiceRoller(() => [3, 3]);
  const held = apply(g, 'p2', 'turn:end');
  assert.deepEqual(held.dwarves.attacks, []);
  assert.equal(unit.frogSpell?.name, 'Жаба ворчун');

  g.turn.activePlayerId = 'p2';
  __setDwarfDiceRoller(() => [3, 4]);
  const released = apply(g, 'p2', 'turn:end');
  assert.equal(unit.frogSpell, null);
  assert.ok(warrior.inventory.includes('art_fairy_glade_005'));
  assert.equal(released.dwarves.frogReleased[0].name, 'Жаба ворчун');
  __setDwarfDiceRoller();
});
...
test('зверь — в схватке нельзя брать карты, но можно телепортироваться', () => {
  ...
  g.turn.dice = [2, 6];
  const warrior = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  const destination = startCell('green', warrior.role);
  warrior.position = neighbors(destination).find(
    (cellId) => !g.characters.some((character) => character.position === cellId),
  );
...
test('Молоток — Кузнец на точке добычи берёт две карты', () => {
  ...
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
  g.deck = ['ore_coarse', 'art_mixed_001'];
...
test('Молоток — два экземпляра на террейне дают три карты и переворачиваются', () => {
  ...
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
  g.deck = ['ore_coarse', 'art_mixed_001', 'gold_nugget'];
...
test('craft Проросший корень — Ковёр шамана возвращается в инвентарь', () => {
  const g = freshGame();
  const shaman = g.characters.find(c => c.owner === 'p1' && c.role === 'S');
  shaman.inventory = ['recipe_sprouted_root', 'shaman_carpet'];
  shaman.crafted = ['shaman_carpet'];
  g.turn.dice = [3, 1];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
  ...
  assert.equal(result.crafted.result, 'art_lake_007');
  assert.deepEqual(result.crafted.materials, ['recipe_sprouted_root']);
  assert.deepEqual(result.crafted.returnedMaterials, ['shaman_carpet']);
  assert.ok(shaman.inventory.includes('shaman_carpet'));
  assert.ok(shaman.inventory.includes('art_lake_007'));
  assert.ok(!shaman.inventory.includes('recipe_sprouted_root'));
  assert.ok(g.discard.includes('recipe_sprouted_root'));
  assert.ok(!g.discard.includes('shaman_carpet'));
  assert.deepEqual(g.turn.usedDice, [true, false]);
});
...
test('craft Топормол — Кузнец открывает по двум смешанным рудам, ветке и двум кубикам 2+', () => {
  const g = freshGame();
  const smith = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  smith.inventory = ['art_dark_forest_013', 'ore_medium', 'ore_coarse', 'art_forest_007'];
  g.turn.dice = [2, 3];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
  ...
  assert.equal(result.crafted.item, 'topormol');
  assert.ok(smith.inventory.includes('topormol'));
  assert.ok(smith.crafted.includes('topormol'));
  assert.ok(!smith.inventory.includes('art_dark_forest_013'));
  assert.ok(!smith.inventory.includes('ore_medium'));
  assert.ok(!smith.inventory.includes('ore_coarse'));
  assert.ok(!smith.inventory.includes('art_forest_007'));
  assert.deepEqual(g.turn.usedDice, [true, true]);
});

test('craft Топормол — Кузнец открывает по руде среднего качества и ветке одним кубиком 4+', () => {
  const g = freshGame();
  const smith = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  smith.inventory = ['art_dark_forest_013', 'art_mixed_001', 'art_forest_012'];
  g.turn.dice = [4, 1];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
  ...
  assert.ok(smith.crafted.includes('topormol'));
  assert.ok(!smith.inventory.includes('art_dark_forest_013'));
  assert.ok(!smith.inventory.includes('art_mixed_001'));
  assert.ok(!smith.inventory.includes('art_forest_012'));
  assert.deepEqual(g.turn.usedDice, [true, false]);
});

test('craft — Деревянный молоток делает чертёж успешным при наличии материалов', () => {
  const g = freshGame();
  const smith = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  smith.inventory = ['art_dark_forest_013', 'ore_coarse', 'ore_medium', 'art_forest_012', 'art_dark_forest_020'];
  g.turn.dice = [1, 1];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
...
test('craft — Иерихон делает чертёж успешным при любых кубиках и материалах', () => {
  const g = freshGame();
  const smith = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  smith.inventory = ['art_dark_forest_013', 'ore_coarse', 'ore_medium', 'art_forest_012', 'irikon'];
  g.turn.dice = [1, 1];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
...
test('craft Деревянный молоток — Кузнец открывает по поленьям и рубину', () => {
  const g = freshGame();
  const smith = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  smith.inventory = ['art_dark_forest_019', 'art_forest_005', 'raw_ruby'];
  g.turn.dice = [2, 2];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
  ...
  const result = apply(g, 'p1', 'action:craft', {
    characterId: smith.id,
    item: 'art_dark_forest_020',
  });

  assert.equal(result.crafted.item, 'art_dark_forest_020');
  assert.ok(smith.inventory.includes('art_dark_forest_020'));
  assert.ok(smith.crafted.includes('art_dark_forest_020'));
  assert.ok(!smith.inventory.includes('art_dark_forest_019'));
  assert.ok(!smith.inventory.includes('art_forest_005'));
  assert.ok(!smith.inventory.includes('raw_ruby'));
  assert.deepEqual(g.turn.usedDice, [true, true]);
});

test('craft Деревянный молоток — без рубина материалов не хватает', () => {
  const g = freshGame();
  const smith = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  smith.inventory = ['art_dark_forest_019', 'art_forest_005'];
  g.turn.dice = [2, 2];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;

  assert.throws(() => apply(g, 'p1', 'action:craft', {
    characterId: smith.id,
    item: 'art_dark_forest_020',
  }), /Не хватает материалов/);
});

test('craft Небрежная кольчуга — Кузнец принимает руду среднего качества', () => {
  const g = freshGame();
  const smith = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  smith.inventory = ['art_dark_forest_003', 'art_mixed_001'];
  g.turn.dice = [3, 3];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
  ...
  const result = apply(g, 'p1', 'action:craft', {
    characterId: smith.id,
    item: 'art_dark_forest_004',
  });

  assert.equal(result.crafted.item, 'art_dark_forest_004');
  assert.ok(smith.inventory.includes('art_dark_forest_004'));
  assert.ok(!smith.inventory.includes('art_mixed_001'));
  assert.deepEqual(g.turn.usedDice, [true, true]);
});

test('craft Щит защита духа — требует две руды высшего качества', () => {
  const g = freshGame();
  const smith = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  smith.inventory = ['art_dark_forest_007', 'art_forest_011'];
  g.turn.dice = [3, 3];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
  ...
  assert.throws(() => apply(g, 'p1', 'action:craft', {
    characterId: smith.id,
    item: 'art_dark_forest_008',
  }), /Не хватает материалов/);
});

test('craft Каска-маска — требует красные ягоды вместе с корой или поленьями', () => {
  const g = freshGame();
  const shaman = g.characters.find(c => c.owner === 'p1' && c.role === 'S');
  shaman.inventory = ['art_recipes_003', 'bark', 'red_berries'];
  g.turn.dice = [2, 2];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
  ...
  const result = apply(g, 'p1', 'action:craft', {
    characterId: shaman.id,
    item: 'art_recipes_004',
  });

  assert.equal(result.crafted.item, 'art_recipes_004');
  assert.ok(shaman.inventory.includes('art_recipes_004'));
  assert.ok(!shaman.inventory.includes('bark'));
  assert.ok(!shaman.inventory.includes('red_berries'));
  assert.deepEqual(g.turn.usedDice, [true, true]);
});

test('craft Маска трехликого — Шаман принимает любой самоцвет из группы камней', () => {
  const g = freshGame();
  const shaman = g.characters.find(c => c.owner === 'p1' && c.role === 'S');
  shaman.inventory = ['art_recipes_009', 'shaman_carpet', 'bark', 'art_lake_001'];
  g.turn.dice = [6, 1];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
  ...
  const result = apply(g, 'p1', 'action:craft', {
    characterId: shaman.id,
    item: 'art_recipes_010',
    dieIndex: 0,
  });

  assert.equal(result.crafted.item, 'art_recipes_010');
  assert.ok(shaman.inventory.includes('art_recipes_010'));
  assert.ok(shaman.crafted.includes('art_recipes_010'));
  assert.ok(!shaman.inventory.includes('art_lake_001'));
  assert.deepEqual(g.turn.usedDice, [true, false]);
});

test('craft Бутыль дил — Проросший корень расходуется, Ковёр шамана возвращается', () => {
  const g = freshGame();
  const shaman = g.characters.find(c => c.owner === 'p1' && c.role === 'S');
  shaman.inventory = ['recipe_dil_bottle', 'shaman_carpet', 'art_lake_007'];
  g.turn.dice = [2, 2];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
  ...
  assert.equal(result.crafted.item, 'dil_bottle');
  assert.deepEqual(result.crafted.materials.sort(), ['art_lake_007', 'recipe_dil_bottle'].sort());
  assert.deepEqual(result.crafted.returnedMaterials, ['shaman_carpet']);
  assert.ok(shaman.inventory.includes('dil_bottle'));
  assert.ok(shaman.inventory.includes('shaman_carpet'));
  assert.ok(!shaman.inventory.includes('art_lake_007'));
  assert.ok(!shaman.inventory.includes('recipe_dil_bottle'));
  assert.ok(g.discard.includes('art_lake_007'));
  assert.ok(g.discard.includes('recipe_dil_bottle'));
  assert.ok(!g.discard.includes('shaman_carpet'));
  assert.deepEqual(g.turn.usedDice, [true, true]);
});

test('craft Заклятие хозяин — Шаман принимает цветной мухомор', () => {
  const g = freshGame();
  const shaman = g.characters.find(c => c.owner === 'p1' && c.role === 'S');
  shaman.inventory = ['art_recipes_025', 'amanita_color', 'art_mixed_003'];
  g.turn.dice = [3, 3];
  g.turn.mode = 'split';
  g.turn.hasRolled = true;
  ...
  const result = apply(g, 'p1', 'action:craft', {
    characterId: shaman.id,
    item: 'art_recipes_026',
  });

  assert.equal(result.crafted.item, 'art_recipes_026');
  assert.ok(shaman.inventory.includes('art_recipes_026'));
  assert.ok(shaman.crafted.includes('art_recipes_026'));
  assert.ok(!shaman.inventory.includes('amanita_color'));
  assert.ok(!shaman.inventory.includes('art_mixed_003'));
  assert.deepEqual(g.turn.usedDice, [true, true]);
});
...
test('move (split) — движение второго персонажа не затирает откат первого (область на каждого)', () => {
  const g = freshGame();
  apply(g, 'p1', 'turn:roll');
  const v = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  const k = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  ...
  const vStart = v.position;
  g.turn.activeDiceCharacterId = v.id;
  g.turn.dice = g.turn.diceByCharacter[v.id];
  g.turn.usedDice = g.turn.usedDiceByCharacter[v.id];
  const vt = availableMoveTargets(g, 'p1', v.id, 0)[0];
  apply(g, 'p1', 'action:move', { characterId: v.id, toCell: vt.cellId, dieIndex: 0 });

  // Второй персонаж ходит своим кубиком
  g.turn.activeDiceCharacterId = k.id;
  g.turn.dice = g.turn.diceByCharacter[k.id];
  g.turn.usedDice = g.turn.usedDiceByCharacter[k.id];
  const kt = availableMoveTargets(g, 'p1', k.id, 0)[0];
  apply(g, 'p1', 'action:move', { characterId: k.id, toCell: kt.cellId, dieIndex: 0 });

  // Область первого персонажа сохранилась и не залочена — откат всё ещё доступен
  assert.ok(g.turn.movementAreaByCharacter[v.id]);
  assert.equal(g.turn.movementAreaByCharacter[v.id].locked, false);
  assert.ok(g.turn.movementAreaByCharacter[k.id]);

  // Откат первого возвращает его на старт, не трогая второго
  apply(g, 'p1', 'turn:resetMove', { characterId: v.id });
  assert.equal(v.position, vStart);
  assert.equal(k.position, kt.cellId);
  assert.equal(g.turn.movementAreaByCharacter[v.id] ?? null, null);
});

test('move (split) — сервер восстанавливает кубик, если клиент прислал ход без dieIndex', () => {
  const g = freshGame();
  apply(g, 'p1', 'turn:roll');
  g.turn.dice = [2, 5]; g.turn.usedDice = [false, false];
  apply(g, 'p1', 'turn:setMode', { mode: 'split' });
  const v = g.characters.find(c => c.owner === 'p1' && c.role === 'V');
  const target = availableMoveTargets(g, 'p1', v.id, 0)[0];
  const result = apply(g, 'p1', 'action:move', { characterId: v.id, toCell: target.cellId });
  assert.equal(v.position, target.cellId);
  assert.equal(result.moved.dieIndex, 0);
  assert.deepEqual(g.turn.usedDice, [true, false]);
});
...
test('draw — тот же персонаж не может добрать второй раз в одном броске', () => {
  const g = freshGame();
  rollAndSplit(g, 'p1');
  const k = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  placeOnResource(k);
...
test('draw — другой персонаж может добрать после первого в том же броске', () => {
  const g = freshGame();
  rollAndSplit(g, 'p1');
  const k = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  const p = g.characters.find(c => c.owner === 'p1' && c.role === 'P');
  placeOnResource(k);
  placeOnResource(p);
  const beforeK = k.inventory.length;
  const beforeP = p.inventory.length;
  g.deck = ['ore_coarse', 'art_mixed_001'];
  apply(g, 'p1', 'action:draw', { characterId: k.id, dieIndex: 0 });
  apply(g, 'p1', 'action:draw', { characterId: p.id, dieIndex: 0 });
  assert.equal(k.inventory.length, beforeK + 1);
  assert.equal(p.inventory.length, beforeP + 1);
  assert.deepEqual(g.turn.drawnCharacterIdsThisTurn, [k.id, p.id]);
});
...
test('ограничения движения/добора сбрасываются с новым броском', () => {
  ...
  assert.deepEqual(g.turn.drawnCharacterIdsThisTurn, []);
});
...
test('Таинственная опушка — вход запускает схватку с фениксом', () => {
  const g = freshGame();
...
test('Таинственная опушка — Жаба ворчун попадает в инвентарь, а не начинает схватку', () => {
  const g = freshGame();
  const { char, result } = stepOnFairyCell(g, ['art_fairy_glade_005']);
  assert.equal(char.beastFight, null);
  assert.equal(result.redEvent.cardId, 'art_fairy_glade_005');
  assert.equal(result.redEvent.name, 'Жаба ворчун');
...
test('Таинственная опушка — пустая колода феникса не создаёт события', () => {
  const g = freshGame();
...
test('Дварфы — скрываются туманом войны в снапшоте игрока', () => {
  const g = freshGame();
  const hiddenDwarf = g.dwarves.units[0];
  const visibleDwarf = g.dwarves.units[1];
  const watcher = g.characters.find(c => c.owner === 'p2');

  hiddenDwarf.position = FAIRY_CELL;
  hiddenDwarf.routeIndex = 30;
  visibleDwarf.position = watcher.position;
  visibleDwarf.routeIndex = 0;

  const snap = snapshotGame(g, 'p2');
  const hiddenView = snap.dwarves.units.find(unit => unit.id === hiddenDwarf.id);
  const visibleView = snap.dwarves.units.find(unit => unit.id === visibleDwarf.id);
  assert.equal(snap.dwarves.routeIndex, null);
  assert.equal(hiddenView.hidden, true);
  assert.equal(hiddenView.position, null);
  assert.equal(hiddenView.routeIndex, null);
  assert.equal(visibleView.hidden, false);
  assert.equal(visibleView.position, watcher.position);
  assert.equal(visibleView.routeIndex, 0);

  const noFog = snapshotGame(g, 'p2', { fogEnabled: false });
  const noFogHiddenView = noFog.dwarves.units.find(unit => unit.id === hiddenDwarf.id);
  assert.equal(noFog.dwarves.routeIndex, g.dwarves.routeIndex);
  assert.equal(noFogHiddenView.hidden, false);
  assert.equal(noFogHiddenView.position, FAIRY_CELL);
  assert.equal(noFogHiddenView.routeIndex, 30);
});
...
test('Перо — старт кузнеца не считается камнем кузнеца', () => {
  const g = freshGame();
  const smith = g.characters.find(c => c.owner === 'p2' && c.role === 'K');
  const smithStart = startCell('red', 'K');
  ...
  const startMove = apply(g, 'p2', 'action:move', {
    characterId: smith.id,
    toCell: smithStart,
    dieIndex: 0,
  });
...
test('сценарий пера — Феникс, маяк, передача Кузнецу и крафт Ирикон', () => {
  ...
  smith.inventory.push('task_irikon');
  g.turn.usedDice = [false, false];
  g.turn.dice = [3, 4];
...
test('craft Ирикон — задание + перо при двух кубиках 3+', () => {
  const g = freshGame();
  const smith = g.characters.find(c => c.owner === 'p1' && c.role === 'K');
  smith.inventory.push('task_irikon', 'gold_feather_own');
  g.turn.dice = [3, 4];
  g.turn.hasRolled = true;
...
test('оружие Деревянный молоток — добавляет 25 обычного урона', () => {
  const g = freshGame();
  const { attacker, target } = prepareAdjacentCombat(g); // кубики [3,4] = 7
  attacker.inventory.push('art_dark_forest_020');

  const r = apply(g, 'p1', 'action:attack', { attackerId: attacker.id, targetId: target.id });

  assert.equal(r.attacked.weaponDamage, 25);
  assert.equal(r.attacked.weaponPiercing, false);
  assert.equal(r.attacked.totalDamage, 32);
  assert.equal(r.attacked.dealtDamage, 32);
...
test('броня Небрежная кольчуга — поглощает 15 обычного урона', () => {
  const g = freshGame();
  const { attacker, target } = prepareAdjacentCombat(g); // урон 7
  placeArmor(g, target, 'art_dark_forest_004');
