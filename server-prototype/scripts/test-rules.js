...
test('dwarves - route index keeps direction on repeated cells and wraps around', () => {
  const g = freshGame();
  const route = dwarfRoute();
  for (const character of g.characters) character.position = null;
  ...
  assert.deepEqual(g.dwarves.units.map(unit => unit.routeIndex), [110, 111, 112, 113, 0]);
  assert.deepEqual(g.dwarves.units.map(unit => unit.position), [route[110], route[111], route[112], route[113], route[0]]);
  assert.equal(new Set(g.dwarves.units.map(unit => unit.position)).size, 5);
  __setDwarfDiceRoller();
});
