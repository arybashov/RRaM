...
test('draw — берёт карту из колоды по рубашке клетки', () => {
  const cases = [
    ...
    { cellId: 'H229', deck: 'forest_trail', pile: 'forest', cardId: 'bark' },
    { cellId: 'H101', deck: 'dark_forest', pile: 'dark_forest', cardId: 'dead_ore' },
    { cellId: 'H010', deck: 'blueprints', pile: 'blueprints', cardId: 'art_dark_forest_003' },
...
test('createGame — красная колода: все красные кроме Ирикона, медведь чаще зверей', () => {
  ...
  assert.equal(g.redDeck.filter(cardId => cardId === 'art_trophy_001').length, 1);
  assert.equal(g.redDeck.filter(cardId => cardId === 'art_trophy_002').length, 1);
...
test('красная клетка — пустая колода пересобирается и даёт красное событие', () => {
  ...
  try {
    ...
    assert.ok(['boar_red', 'wolf', 'beast_bear', 'art_trophy_001', 'art_trophy_002', 'axe_sun', 'task_irikon'].includes(result.redEvent.cardId));
...
test('оружие Деревянный молоток — добавляет 10 обычного урона', () => {
  const g = freshGame();
  const { attacker, target } = prepareAdjacentCombat(g); // кубики [3,4] = 7
  ...
  assert.equal(r.attacked.weaponDamage, 10);
  assert.equal(r.attacked.weaponPiercing, false);
  assert.equal(r.attacked.totalDamage, 17);
  assert.equal(r.attacked.dealtDamage, 17);
