...
const CARD_FACE_ART = {
  ...
  blueprint_hammer: 'base/blacksmith/hammer-blueprint-v1',
  ...
  blueprint_club: 'base/warrior/club-blueprint-v1',
  ...
  gold_feather: 'fairy-glade/gold-feather-v1',
};
...
const CARD_CATALOG_META = Object.freeze({
  ...
  lake_frog: { deck: "lake", type: "special", copies: 1, name: "Озёрная лягушка" },
  raw_ruby: { deck: "lake", type: "ingredient", copies: 1, name: "Необработанный рубин" },
  ...
  marvo: { deck: "recipes", type: "provocation", copies: 2, name: "Марво трос" },
  blueprint_club: { deck: "blueprints", type: "blueprint", copies: 2, name: "Чертёж на дубину" },
  blueprint_hammer: { deck: "blueprints", type: "blueprint", copies: 2, name: "Чертёж на молоток" },
  ...
  gold_feather: { deck: "fairy_glade", type: "special", copies: 0, name: "Золотое перо", desc: "Маяк: персонаж-носитель всегда виден всем (игнорирует туман). Материал для Молота Иерихон.", public: true },
  ...
  bp_hammer_base: { role: "K", type: "blueprint", copies: 1, name: "Базовый чертёж на молоток", desc: "Материалы: смешанная железная руда. Испытание: два кубика, каждый не меньше 3. Открывает Молоток." },
  ...
  bp_club_base: { role: "V", type: "blueprint", copies: 1, name: "Базовый чертёж на дубину", desc: "Материалы: убить кабана, медведя или волка → шкуру очищает шаман → очищенной шкурой открыть Дубину." },
  club: { role: "V", type: "weapon", copies: 1, name: "Дубина", desc: "Класс: воин. В начале хода враг теряет 10 HP. Против зверя кубик 4+ побеждает его одной атакой.", locked: true, public: true },
  ...
  recipe_shaman_carpet: { role: "S", type: "recipe", copies: 1, name: "Рецепт на Ковёр шамана", desc: "Материалы: клубок ×1 + шкура медведя ×1. Кубик 1 раз не менее 3. Открывает Ковёр шамана." },
  shaman_carpet: { role: "S", type: "tool", copies: 1, name: "Ковёр шамана", desc: "Каждое начало хода Шаман восстанавливает 2 HP. Также применяется в обрядах и изделиях по рецептам.", locked: true },
...
const CARD_DECK_BY_ID = Object.freeze({
  ...
  yarn: 'base',
  ore_medium: 'mixed',
  blueprint_club: 'blueprints',
  blueprint_hammer: 'blueprints',
  boar_forest: 'forest',
  sheep_ram: 'sheep',
  sheep_wool: 'sheep',
  sheep_hide_r: 'sheep',
...
const APP_VERSION = '20260617-07'; // = BUILD_VERSION (сервер) и ?v= в index.html; бампать через scripts/bump-version.mjs
...
function getDrawCharacter(preferred = getSelChar()) {
  ...
  return drawableCharacters()[0] ?? preferred;
}

function carriesGoldFeather(char) {
  return Boolean(char?.inventory?.some(card => (card.id ?? card) === 'gold_feather'));
}
...
function renderEncyclopediaSections(cardEntries) {
  return `<section class="encyclopedia-section encyclopedia-section-characters">`
...
function renderCardFace(card, size = 'inventory') {
  ...
  return art
    ? `<img class="inventory-card-art generated-card-${cardClassToken(size)}" src="./assets/cards/${art}.png" alt="${escapeHtml(card.name)}" draggable="false" />`
...
function directCardAction(mode) {
  ...
  let char = getSelChar();
  if (mode === 'draw') char = getDrawCharacter(char);
  if (!char) { addLog('Сначала выберите персонажа.', { type: 'err' }); render(); return; }
  if (mode === 'draw' && !canDrawWithCharacter(char)) {
    showActionWarning('Взять карту можно только на точке ресурса.');
    render();
    return;
  }
  if (mode === 'draw' && char.id !== selectedCharId) {
    selectedCharId = char.id;
  }
...
function renderDice() {
  ...
  if (drawModeBtn) {
    ...
    const onResource = canDrawWithCharacter(sel) || drawableCharacters().length > 0;
    drawModeBtn.disabled = !(myTurn && free && !g.turn.drawnThisTurn && onResource);
    drawModeBtn.title = g.turn.drawnThisTurn
      ? 'Карту в этом броске уже брали — второй кубик потратьте на другое действие'
      : !free
        ? 'Для добора нужен свободный кубик'
      : !onResource
        ? 'Взять карту можно только на точке ресурса'
...
function renderBoard() {
  ...
  for (const char of charactersByDepth) {
    ...
    if (isOwn) {
      ...
      const onEnemyClick = (event) => {
        ...
        if (myInCombat) {
          ...
          showActionWarning('Для удара нужны оба свободных кубика.');
          return;
        }
        const selectedPos = characterPosition(selectedAttacker);
        ...
        if (
          ...
          && selectedPos
          && enemyPos
          && hexNeighbors(selectedPos).includes(enemyPos)
        ) {
    ...
    if (char.beacon) {
      const beacon = document.createElementNS(svgNS, 'text');
      beacon.setAttribute('class', 'token-beacon');
      beacon.setAttribute('text-anchor', 'middle');
      beacon.setAttribute('y', (-HEX_R * 0.92).toFixed(1));
      beacon.style.fontSize = `${(HEX_R * 0.7).toFixed(1)}px`;
      beacon.textContent = '🪶';
...
const CRAFT_RECIPES = {
  ...
  irikon: { role: 'K', via: 'blueprint_irikon', result: 'irikon', label: 'Молот Иерихон', materials: [['task_irikon'], ['gold_feather']], diceCount: 2, diceMin: 3 },
};
...
const beastCardArt = (id) => {
  ...
  return art ? `./assets/cards/${art}.png` : null;
};
...
function cellClassName(cell) {
  ...
  if (cell?.terrain === 'start' && cell?.role === 'K') classes.push('blacksmith-stone');
...
function handleActionResult(result) {
  ...
  if (result.attacked) {
    ...
    if (a.griffinDamage > 0) {
      ...
      addLog(`${attackerName} атаковал ${targetName}: кубики ${a.damage} + ${a.weaponName} ${a.weaponDamage} = ${a.totalDamage}.`, { type: 'err' });
    } else {
      showToast(`⚔️ Атака: ${a.damage} урона!`, 'danger');
      addLog(`${attackerName} атаковал ${targetName}: ${a.damage} урона`, { type: 'err' });
    }

    if (a.armorAbsorbed > 0) {
      addLog(`Броня ${targetName} поглотила ${a.armorAbsorbed} урона (нанесено ${a.dealtDamage}).`, { type: 'sys' });
    }
  ...
  for (const char of nextG.characters) {
    ...
    if (prevChar && char.hp < prevChar.hp) {
      ...
      const myClubber = opp && opp.owner === myPlayerId && opp.role === 'V'
        && (nextG.terrainCards ?? []).some(card =>
          card.characterId === opp.id && card.card?.id === 'club');
