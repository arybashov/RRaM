...
const ENCYCLOPEDIA_RULES = Object.freeze([
  ...
  {
    title: 'Бой',
    text: 'Атака по соседу тратит оба кубика. Оружие добавляет урон, броня поглощает, ловушки срабатывают рубашкой вверх.',
  },
  {
    title: 'Перо и Ирикон',
...
const CARD_USAGE_DESCRIPTIONS = Object.freeze({
  ...
  bear_hide: 'Трофей со зверя. Нужна Шаману для Ковра шамана; также может быть шкурой в рецепте Жеста.',
  bark: 'Броня. Выложите лицом вверх на свою фишку. Пока карта открыта, она поглощает 5 урона от входящей атаки.',
  gold_nugget: 'Лечение. В инвентаре раненого персонажа нажмите «Лечиться»: карта одноразово восстанавливает до 20 HP, но не выше 100.',
  ...
  dead_ore: 'Особый ресурс. В инвентаре нажмите кнопку выбранной колоды: Неживая руда расходуется и берёт одну карту из mixed, forest, dark forest, sheep или lake. В бою не применяется.',
  ...
  beast_bear: 'Сильный красный зверь. В схватке кусает больнее остальных; победа даёт шкуру медведя для Ковра шамана и других рецептов.',
  ...
  irikon: 'Молот Иерихон. Оружие Кузнеца: при атаке добавляет 35 урона без учёта защиты. Можно держать в инвентаре или выложить лицом вверх.',
  lake_frog: 'Заклятие Шамана. В схватке со зверем нажмите «На зверя», чтобы сразу победить. В бою с игроком нажмите «На врага», чтобы отключить оружие цели до броска суммы 8+.',
  ...
  marvo: 'Обряд трёх. Шаман использует свободный кубик: Марво наносит значение кубика ×10 урона всем врагам рядом, если у Шамана есть активное оружие.',
  ...
  club: 'Оружие Воина. В бою с игроком наносит 10 HP в начале хода врага; против зверя кубик 4+ сразу побеждает зверя.',
  ...
  recipe_shaman_carpet: 'Рецепт Шамана. Клубок и шкура медведя при кубике 3+ открывают Ковёр шамана.',
  shaman_carpet: 'Инструмент Шамана. В начале каждого хода Шаман восстанавливает 2 HP за каждый активный Ковёр, но не выше 100.',
...
const CARD_CATALOG_META = Object.freeze({
  ...
  marvo: { deck: "recipes", type: "provocation", copies: 2, name: "Марво трос", desc: "Обряд трёх. Крафтится Шаманом из Полянки мухоморов и Озёрной лягушки. Боевой эффект будет подключён отдельно." },
...
function onResume() {
  ...
  const stale = !ws
    ...
    || (Date.now() - lastServerMsgAt) > HEARTBEAT_MS + 1000; // пропущен ≥1 цикл heartbeat
  if (stale) {
    forceReconnect();
  } else {
...
function drawDieIndex() {
  ...
  if (!used[selectedDieIdx]) return selectedDieIdx;
  const other = selectedDieIdx === 0 ? 1 : 0;
  return used[other] ? null : other;
}
...
function directCardAction(mode) {
  ...
  if (!getDice()) { showActionWarning('Сначала бросьте кубики.'); render(); return; }
  if (mode === 'draw' && getGame().turn.drawnThisTurn) {
    showActionWarning('Карту в этом броске уже брали — второй кубик потратьте на другое действие.');
    render();
    return;
  }
  let char = getSelChar();
  ...
  if (mode === 'draw' && !canDrawWithCharacter(char)) {
    showActionWarning(plannedResourceDraw(char)
      ? 'Дойдите до ресурса одним кубиком (раздельные кубики), затем нажмите «Взять».'
