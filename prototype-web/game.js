...
let encyclopediaPreviewSeq = 0;
const encyclopediaPreviewCards = new Map();
...
const APP_VERSION = '20260701-24'; // = BUILD_VERSION (сервер) и ?v= в index.html; бампать через scripts/bump-version.mjs
...
function buildEncyclopediaOverlay() {
  ...
  encyclopediaEl.addEventListener('click', (e) => {
    ...
    const card = e.target.closest('.encyclopedia-card-toggle[data-zoom-key]');
    if (!card || !encyclopediaEl.contains(card)) return;
    const previewCard = encyclopediaPreviewCards.get(card.dataset.zoomKey);
    if (previewCard) showGameCard(GameCard.preview(previewCard));
  });
}
...
function openEncyclopedia() {
  ...
  encyclopediaPreviewCards.clear();
  encyclopediaPreviewSeq = 0;
...
function renderEncyclopediaCard(card) {
  ...
  const numberLabel = Number.isFinite(card.encyclopediaNumber)
    ...
    : '№---';
  const previewKey = `card-${encyclopediaPreviewSeq++}`;
  encyclopediaPreviewCards.set(previewKey, card);
  ...
  return `<article class="encyclopedia-card card-${cardClassToken(meta.type)} deck-${cardClassToken(meta.deck)}">`
    + `<button class="encyclopedia-card-toggle" type="button" data-zoom-key="${escapeHtml(previewKey)}" aria-label="Open ${escapeHtml(meta.name)}">`
...
class GameCard {
  ...
  actions(refresh, { previewFlip = null } = {}) {
    const canUseHandActions = this.isOwn && this.location === 'inventory' && this.ownerCharacter;
    const canUsePreviewActions = this.location === 'preview' && !this.card?.exhausted && !this.card?.locked && !this.card?.hidden;
    ...
    const previewFaceDown = this.hidden;

    return {
      flip: canUsePreviewActions
        ? () => {
            this.hidden = !this.hidden;
            this.faceDown = this.hidden;
            if (previewFlip) previewFlip();
            else refresh?.();
          }
        : canUseTerrainActions
          ? () => {
        ...
        : (canUseHandActions && faceKey && !this.card?.exhausted && !this.card?.locked && !this.card?.hidden)
          ...
          : null,
      flipLabel: (canUsePreviewActions ? previewFaceDown : (canUseTerrainActions ? this.faceDown : handFaceDown)) ? 'Лицом вверх' : 'Рубашкой вверх',
...
function showGameCard(gameCard, { closeLabel = 'Закрыть', dim = false } = {}) {
  ...
  const updateActions = () => {
    setOpenCardActions({
      ...gameCard.actions(render, { previewFlip: applyPreviewFlip }),
      closeLabel,
    });
  };
  const applyPreviewFlip = () => {
    const flipCard = display.querySelector('.event-preview-flip-card');
    if (!flipCard) {
      render?.();
      return;
    }
    flipCard.classList.toggle('is-flipped', gameCard.hidden);
    updateActions();
  };
  render = () => {
    const isPreview = gameCard.location === 'preview';
    display.classList.toggle('has-preview-flip', isPreview);
    display.innerHTML = isPreview
      ? renderPreviewFlipCard(gameCard)
      : renderCard(gameCard.renderCard, gameCard.renderIndex, true, gameCard.characterId);
    const cardEl = display.querySelector('.card');
    if (cardEl && dim) cardEl.style.opacity = '0.7';
    if (!isPreview && cardEl && gameCard.upsideDown && !gameCard.faceDown) cardEl.classList.add('card-upside-down');
    updateActions();
  };
  render();
...
function renderPreviewFlipCard(gameCard) {
  const card = gameCard.card ?? {};
  const frontCard = { ...card, hidden: false };
  const visualMeta = cardVisualMeta(frontCard);
  const typeLabel = CARD_TYPE_LABELS[visualMeta.type] ?? visualMeta.type;
  const deckLabel = CARD_DECK_LABELS[visualMeta.deck] ?? visualMeta.deck;
  const desc = cardDescription({ ...frontCard, ...visualMeta }, deckLabel, typeLabel);
  const backArt = cardBackArtForCard(frontCard);
  const faceArt = cardFaceArt(frontCard);
  const face = faceArt
    ? `<img class="inventory-card-art" src="${cardFaceArtUrl(faceArt)}" alt="${escapeHtml(visualMeta.name)}" draggable="false" />`
    : renderGeneratedCardArt(frontCard, 'inventory');
  const flipped = gameCard.hidden ? ' is-flipped' : '';
  return `<div class="event-preview-flip-card${flipped}">`
    + `<div class="event-preview-flip-face event-preview-flip-front">${face}</div>`
    + `<div class="event-preview-flip-face event-preview-flip-back"><img src="${backArt}" alt="${escapeHtml(visualMeta.name)}" draggable="false" /></div>`
    + `</div>`
    + (desc ? `<div class="card-desc event-preview-card-desc">${escapeHtml(desc)}</div>` : '');
}
...
function showCardReveal(cardOrCards) {
  const cards = (Array.isArray(cardOrCards) ? cardOrCards : [cardOrCards]).filter(Boolean);
  ...
  professionDrawEl.innerHTML =
    '<div class="profdraw-stage' + (cards.length > 1 ? ' profdraw-stage-multi' : '') + '">' +
      '<div class="profdraw-row">' +
        cards.map((card, index) =>
          '<div class="profdraw-card" data-reveal-index="' + index + '">' +
            '<div class="profdraw-face profdraw-back"><img src="' + cardBackArtForCard(card) + '" alt="" draggable="false" /></div>' +
            '<div class="profdraw-face profdraw-front">' + renderCard(card, 999 + index, true) + '</div>' +
          '</div>').join('') +
      '</div>' +
      '<div class="profdraw-hint" id="profdrawHint"></div>' +
    '</div>';

  const cardEls = [...professionDrawEl.querySelectorAll('.profdraw-card')];
  const hint = professionDrawEl.querySelector('#profdrawHint');
  cardEls.forEach((cardEl) => cardEl.classList.remove('is-flipped', 'is-leaving'));
  hint.textContent = cards.length > 1
    ? 'Нажмите каждую карту, чтобы перевернуть'
  ...
  const onClick = (e) => {
    ...
    if (closing) return;
    if (onCard && !onCard.classList.contains('is-flipped')) {
      onCard.classList.add('is-flipped');
      flipped.add(onCard.dataset.revealIndex);
      hint.textContent = flipped.size < cards.length ? 'Раскройте все карты' : '';
      return;
    }
    if (flipped.size < cards.length) return;
    closing = true;
    cardEls.forEach((cardEl) => cardEl.classList.add('is-leaving'));
...
function handleActionResult(result) {
  ...
  if (result.redEvent) {
    ...
    if (ev.beast) {
      ...
      showCardReveal({
        ...
        type: ev.type,
        desc: ev.desc,
        sourceDeck: ev.sourceDeck ?? 'red',
        sourceBack: ev.sourceBack ?? ev.sourceDeck ?? 'red',
      });
  ...
  if (result.drawn) {
    ...
    const revealCards = Array.isArray(d.cards) && d.cards.length > 0
      ? d.cards.map(item => ({
          id: item.card,
          name: item.name,
          type: item.type,
          desc: item.desc,
          sourceDeck: item.sourceDeck ?? d.deck ?? null,
          sourceBack: item.sourceBack ?? item.sourceDeck ?? d.deck ?? null,
        }))
      : [card];
    showCardReveal(revealCards);
