...
const APP_VERSION = '20260617-24'; // = BUILD_VERSION (сервер) и ?v= в index.html; бампать через scripts/bump-version.mjs
...
function renderBoard() {
  ...
  for (const char of charactersByDepth) {
    ...
    if (char.beacon) {
      const featherWidth = HEX_R * 0.78;
      const featherHeight = featherWidth * (512 / 341);
      const featherY = (-figureHeight * 0.7 - featherHeight * 0.52);
      ...
      beaconGlow.setAttribute('rx', (featherWidth * 0.74).toFixed(2));
      beaconGlow.setAttribute('ry', (featherHeight * 0.56).toFixed(2));
      g.appendChild(beaconGlow);

      const beacon = document.createElementNS(svgNS, 'image');
      const featherCardId = goldFeatherCardId(char) ?? 'gold_feather_own';
      const featherHref = cardFaceArtUrl(CARD_FACE_ART[featherCardId] ?? CARD_FACE_ART.gold_feather_own);
