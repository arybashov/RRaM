...
const APP_VERSION = '20260614-17'; // = BUILD_VERSION (сервер) и ?v= в index.html; бампать через scripts/bump-version.mjs
...
function renderDice() {
  ...
  if (teleportModeBtn) {
    ...
    teleportModeBtn.title = hasBeads && hasFreeDie
...
function renderInventory() {
  ...
  const craftInfo = Object.entries(CRAFT_RECIPES)
    ...
    if (has(r.via) && matsReady) {
      ...
      return `<div class="craft-info">🔨 Есть материалы — можно ${action.toLowerCase()} «${r.label}»! `
        + `<button class="craft-btn" data-item="${item}" ${(isMyTurn() && diceReady) ? '' : 'disabled'}>${action} «${r.label}»</button>`
