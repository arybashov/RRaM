...
function combatHpView(char) {
  ...
  const total = hp + armor;
  const title = armor > 0
    ? `${hp} HP + броня ${armor} = ${total} HP`
    : `${hp} HP`;
  return { hp, armor, total, boosted: armor > 0, title };
}
...
const APP_VERSION = '20260625-25'; // = BUILD_VERSION (сервер) и ?v= в index.html; бампать через scripts/bump-version.mjs
...
const CRAFT_RECIPES = {
  ...
  art_dark_forest_020: {
    ...
    options: [
      { role: 'K', materials: [['art_forest_005']], diceCount: 2, diceMin: 2 },
      { role: 'K', materials: [['art_forest_012'], ['raw_ruby']], diceCount: 2, diceMin: 2 },
      { role: 'K', materials: [['art_forest_005']], diceCount: 1, diceMin: 3 },
      { role: 'K', materials: [['art_forest_012'], ['raw_ruby']], diceCount: 1, diceMin: 3 },
    ],
  },
  axe_sun_ext: {
    role: 'K',
...
function renderCombatBoardElements(fogCircles) {
  ...
  for (const [uid, tc] of terrainCards) {
    ...
    const art = cardFaceArt(tc.cardData ?? cardId);
