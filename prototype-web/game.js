...
const CARD_FACE_ART = {
  ...
  art_dark_forest_013: 'dark-forest/blueprint-topormol-v1',
...
function activeArmorCardsForCharacter(characterId) {
  ...
  return [...terrainCards.values()].filter((card) =>
    ...
    && !card.faceDown
    && card.cardId
    && ACTIVE_ARMOR_STATS[card.cardId]);
}
...
const APP_VERSION = '20260626-7'; // = BUILD_VERSION (сервер) и ?v= в index.html; бампать через scripts/bump-version.mjs
...
function optionMaterialsText(option) {
  return option.materials.length
...
function renderCombatStatsSummary(char, activeTerrainCards) {
  ...
  const dice = getDice(char.id);
  const diceDamage = dice ? dice.reduce((sum, value) => sum + (Number(value) || 0), 0) : 0;
  ...
  const activeGriffins = char.role === 'O'
    ...
    : 0;
  let griffinDamage = 0;
  if (activeGriffins > 0 && diceDamage > 0) {
    const perGriffin = diceDamage === 2 ? 10
      : diceDamage === 3 ? 20
      : diceDamage === 4 ? 25
      : diceDamage >= 5 ? 30
      : 0;
    griffinDamage = perGriffin * activeGriffins;
  }

  const shamanCarpets = char.role === 'S'
    ? activeTerrainCards.filter((card) => card.id === 'shaman_carpet').length
    : 0;
  const firewoodCount = activeTerrainCards.filter((card) => card.id === FIREWOOD_CARD_ID).length;
  const firewoodMultiplier = firewoodCount > 0 ? 2 : 1;
  const weaponDamage = bestWeapon?.damage ?? 0;
  const flatDamage = clubDamage + weaponDamage + griffinDamage;
  const knownDamage = (diceDamage + flatDamage) * firewoodMultiplier;
  const bonusDamage = flatDamage * firewoodMultiplier;
  const beastDefense = armorTotal + (firewoodCount > 0 ? 5 : 0);

  const chips = [];
  if (diceDamage > 0 || flatDamage > 0 || firewoodMultiplier > 1 || activeGriffins > 0) {
    const label = diceDamage > 0
      ? `Урон ${knownDamage}`
      : flatDamage > 0
        ? `Урон +${bonusDamage}${firewoodMultiplier > 1 ? ' ×2' : ''}`
        : `Урон ×${firewoodMultiplier}`;
    const titleParts = [];
    if (diceDamage > 0) titleParts.push(`кубики ${diceDamage}`);
    if (weaponDamage > 0) titleParts.push(`${bestWeapon.name} +${weaponDamage}${bestWeapon.piercing ? ' без защиты' : ''}`);
    if (clubDamage > 0) titleParts.push(`Дубина +${clubDamage}`);
    if (griffinDamage > 0) titleParts.push(`Гриффон +${griffinDamage}`);
    else if (activeGriffins > 0) titleParts.push('Гриффон: бонус зависит от суммы кубиков');
    if (firewoodMultiplier > 1) titleParts.push('Поленья: итоговый урон x2');
    chips.push({
      kind: 'attack',
      label,
  ...
  if (beastDefense > armorTotal) {
    chips.push({
      kind: 'defense',
      label: `Против зверя -${beastDefense}`,
      title: `Броня ${armorTotal} + Поленья -5`,
    });
  }
  if (shamanCarpets > 0) {
    chips.push({
  ...
  return '<div class="combat-stats">'
    + '<div class="combat-stats-title">Итоговые характеристики</div>'
...
const FALLBACK_CRAFT_RECIPES = {
  ...
  topormol: {
    ...
    label: 'Топормол',
    options: [
      { role: 'K', materials: [['ore_medium', 'ore_coarse'], ['ore_medium', 'ore_coarse']], diceCount: 2, diceMin: 2 },
      { role: 'K', materials: [['art_mixed_001'], ['art_forest_012', 'art_forest_007']], diceCount: 2, diceMin: 2 },
      { role: 'K', materials: [['ore_medium', 'ore_coarse'], ['ore_medium', 'ore_coarse']], diceCount: 1, diceMin: 4 },
...
function handleActionResult(result) {
  ...
  if (result.redEvent) {
    ...
    if (ev.beast) {
      ...
      showCardReveal({ id: ev.cardId, name: ev.name, type: ev.type, desc: ev.desc });
  ...
  if (result.oakAcornsUsed) {
    const d = result.oakAcornsUsed;
    const deckLabel = CARD_DECK_LABELS[d.deck] ?? d.deck;
    showCardReveal({ id: d.card, name: d.name, type: d.type, desc: d.desc });
