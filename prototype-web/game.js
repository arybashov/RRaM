...
const APP_VERSION = '20260619-11'; // = BUILD_VERSION (сервер) и ?v= в index.html; бампать через scripts/bump-version.mjs
...
function handleCellClick(targetId) {
  ...
  if (usesServerPositions()) {
    ...
    if (moveMode === 'moveDie') {
      const dieIndex = firstFreeDieIndexFor(char.id);
      if (dieIndex == null) {
        showActionWarning('Нет свободного кубика для движения.');
        return;
      }
      selectedDieIdx = dieIndex;
