...
const APP_VERSION = '20260618-11'; // = BUILD_VERSION (сервер) и ?v= в index.html; бампать через scripts/bump-version.mjs
    ...
    if (isOwn) {
      ...
      const onEnemyClick = (event) => {
        ...
        if (myInCombat) {
          ...
          showActionWarning(hasFreeDie() ? 'Кликните по противнику ещё раз, чтобы ударить.' : 'Для удара нужен свободный кубик.');
          return;
        }
