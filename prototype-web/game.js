...
const APP_VERSION = '20260620-06'; // = BUILD_VERSION (сервер) и ?v= в index.html; бампать через scripts/bump-version.mjs
...
function onInventoryClick(e) {
  ...
  if (e.target.closest('.card-discard-btn') && char && item) {
    e.stopPropagation();
    const cardName = item.name ?? getCardName(item.id ?? item);
