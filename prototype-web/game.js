...
const CARD_CATALOG_META = Object.freeze({
  ...
  topormol: { deck: "dark_forest", type: "weapon", copies: 0, name: "Топормол", desc: "Оружие. Атака по врагу: −25 HP без учёта защиты. Многоразовое." },
...
const APP_VERSION = '20260625-17'; // = BUILD_VERSION (сервер) и ?v= в index.html; бампать через scripts/bump-version.mjs
...
function craftStatusForCharacter(char, item, recipe) {
  ...
  const best = optionStatuses.find(status => status.missingMaterials.length === 0)
...
function cardBoxCardFromElement(cardEl) {
  ...
  if (typeof card === 'string') {
    const meta = CARD_CATALOG_META[card] ?? {};
