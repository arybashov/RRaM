...
const APP_VERSION = '20260613-2'; // = BUILD_VERSION (сервер) и ?v= в index.html; бампать через scripts/bump-version.mjs
...
function renderDice() {
  ...
  const resettable = Boolean(g.turn.movementArea && !g.turn.movementArea.locked && myTurn);
  dieButtons.forEach((btn, i) => {
    // «🎲» только когда реально можно бросить; потрачено всё — «–»
    btn.textContent = dice ? dice[i] : (canRoll ? '🎲' : '–');
    btn.disabled    = dice ? (!myTurn || (used[i] && !resettable)) : !canRoll;
    btn.className   = 'die';
...
function renderInventory() {
  ...
  const hideInfo = canProcessHide
    ? `<div class="craft-info">🧵 Шаман может обработать «Шкуру убитого зверя» → очищенная. `
  ...
  const recipeEntry = Object.entries(CRAFT_RECIPES).find(([, r]) => r.role === char.role);
  let craftInfo = '';
  if (recipeEntry) {
    const [item, r] = recipeEntry;
  ...
  inventoryEl.innerHTML = inv.length
    ? beastInfo + hideInfo + craftInfo + inv.map((c, i) => renderCard(c, i)).join('')
    : (beastInfo || 'Инвентарь пуст.');
  inventoryEl.querySelector('#craftBtn')?.addEventListener('click', (e) => {
    e.stopPropagation(); // не раскрывать карту под кнопкой
    wsSend('action:craft', { characterId: char.id, item: e.currentTarget.dataset.item });
  });
...
const RAW_HIDE_IDS = ['raw_hide', 'raw_hide_red'];
...
function renderCombatScene(mine, enemyOverride = null) {
  ...
  const myCards = combatCardsHtml(mine.inventory);
  combatEl.querySelector('#combatMine').innerHTML = `
    <div class="cb-mine-layout">
      ${combatMineCharHtml(mine, myCards)}
    </div>`;
}
...
function combatMineCharHtml(mine, cardsHtml) {
  return `
    <div class="cb-char">
      <img class="cb-char-card" src="${charCardArt(mine.role)}" alt="${ROLE_NAMES[mine.role]}" />
      <div class="cb-foe-info cb-mine-info">
        <div class="cb-name">${ROLE_NAMES[mine.role]} · вы</div>
        <div class="cb-hpbar"><div style="width:${Math.max(0, mine.hp)}%"></div></div>
        <div class="cb-hp">${mine.hp} HP</div>
        <div class="cb-cards">${cardsHtml}</div>
      </div>
    </div>`;
}

function combatCardsHtml(inventory) {
  return (inventory ?? []).map(c => {
    const card = typeof c === 'string'
      ? { name: c, type: 'unknown', locked: false }
      : c;
    const art = CARD_FACE_ART[card.id];
    if (art) {
      return `<div class="cb-card cb-card-face card-${card.type ?? 'unknown'}${card.locked ? ' card-locked' : ''}" title="${escapeHtml(card.name)}">`
        + `<img src="./assets/cards/${art}.png" alt="${escapeHtml(card.name)}" draggable="false" />`
        + (card.locked ? '<span class="cb-card-lock">🔒</span>' : '')
        + `</div>`;
    }
